...
describe("Subscription Status Query", () => {
	it("应该返回用户的订阅信息", async () => {
		...
		expect(sub).toBeDefined();
		expect(sub?.stripePriceId).toBe("price_yearly_pro");
		expect(sub?.status).toBe("active");
	});
...
describe("Subscription Update", () => {
	...
	it("应该更新 Price ID（升级/降级计划）", async () => {
		...
		const updated = await getUserSubscription(testUser.id);
		expect(updated?.stripePriceId).toBe("price_monthly_pro");
	});
});

// ============================================
// Stripe Customer ID 测试
// ============================================

describe("Stripe Customer ID", () => {
	it("应该存储 Stripe Customer ID", async () => {
		const testUser = await createTestUser();
		createdUserIds.push(testUser.id);
		...
		expect(updated?.stripeCustomerId).toBe(stripeCustomerId);
	});

	it("新用户应该没有 Stripe Customer ID", async () => {
		const testUser = await createTestUser();
		createdUserIds.push(testUser.id);
		...
		expect(dbUser?.stripeCustomerId).toBeNull();
	});

	it("应该更新 Stripe Customer ID", async () => {
		const testUser = await createTestUser();
		createdUserIds.push(testUser.id);
		...
		await testDb
			.update(user)
			.set({ stripeCustomerId: oldCustomerId })
			.where(eq(user.id, testUser.id));
...
describe("Subscription User Association", () => {
	it("不同用户应该有独立的订阅", async () => {
		...
		expect(sub1?.stripePriceId).toBe("price_basic");
		expect(sub1?.status).toBe("active");
		expect(sub2?.stripePriceId).toBe("price_pro");
