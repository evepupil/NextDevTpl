...
describe("Storage User Isolation", () => {
  ...
  it("用户 ID 可以出现在路径的任意位置", async () => {
    ...
    // ID 在路径中间
    expect(
      validateUserOwnership(`avatars/${testUser.id}/photo.jpg`, testUser.id)
    ).toBe(true);
    // ID 在文件名中
    expect(
      validateUserOwnership(`avatars/photo-${testUser.id}.jpg`, testUser.id)
    ).toBe(true);
    // ID 在开头
    expect(
      validateUserOwnership(`${testUser.id}/avatars/photo.jpg`, testUser.id)
    ).toBe(true);
  });
});
...
describe("Storage Security Attack Vectors", () => {
  it("应该防止路径遍历攻击", async () => {
    ...
    // 尝试使用 .. 遍历目录
    expect(
      validateKeyFormat(`avatars/${testUser.id}/../../../etc/passwd`)
    ).toBe(false);
