...
| Open Graph image template | `src/components/seo/og-image-template.tsx` |
...
Keep route-specific composition in `src/app` and reusable business logic inside
the feature module. `manifest.ts` declares dependencies, routes, schema, and
translation namespaces; `index.ts` exports the public surface. Call external
services through `src/services`.
...
| PSEO data | `src/features/pseo/lib/pseo-data.ts` |
