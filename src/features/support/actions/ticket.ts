 ...
 * 获取用户的工单列表
 */
export const getMyTicketsAction = protectedAction.action(async ({ ctx }) => {
 ...
 * 只能查看自己的工单
 */
export const getTicketDetailAction = protectedAction
 ...
 * 获取所有工单列表 (管理员)
 */
export const getAllTicketsAction = adminAction.action(async () => {
  const tickets = await db
    .select({
      id: ticket.id,
      subject: ticket.subject,
 ...
 * 管理员可以查看任何工单
 */
export const getAdminTicketDetailAction = adminAction
  .schema(addTicketMessageSchema.pick({ ticketId: true }))
